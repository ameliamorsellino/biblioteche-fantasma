...
DEV='https://biblioteche-fantasma.invalid/'
ONTO=DEV+'ontology/'
RES=DEV+'resource/'
...
class MetricsSink:
    ...
    def triple(self,s,p,o):
        ...
        if isinstance(s,URIRef) and str(s).startswith(DEV): self.local_subjects.add(s)
        if isinstance(o,URIRef) and str(o).startswith(DEV): self.local_objects.add(o)
        ...
        if p in {OWL.sameAs, RDFS.seeAlso, SKOS.exactMatch, SKOS.closeMatch}:
            ...
            if isinstance(o,URIRef) and not str(o).startswith(DEV): self.external_link_targets[p]+=1
...
def main():
    ...
    meta_local_subjects={s for s in meta.subjects() if isinstance(s,URIRef) and str(s).startswith(DEV)}
    ont_local_subjects={s for s in ont.subjects() if isinstance(s,URIRef) and str(s).startswith(DEV)}
    defined=data_sink.local_subjects|meta_local_subjects|ont_local_subjects
    # Ignore local distribution downloadURL pseudo-endpoints: declared provisional document URLs, not RDF entities.
    dangling={u for u in data_sink.local_objects if u not in defined and not str(u).startswith(DEV+'distribution/')}
    ...
    with report.open('w',encoding='utf-8') as f:
        ...
        f.write(f'- dangling local resource targets (excluding provisional download URLs): **{len(dangling):,}**\n')
