...
| CQ9 | Quali comuni con elevata perdita demografica hanno la maggiore quota di record bibliotecari con stato ICCU incluso nel perimetro problematico? | `bf:populationChangePercent2019_2025`, osservazioni di stato, municipio | “Problematiche” segue esclusivamente il criterio `include_in_main_analysis=True` definito nella pipeline di cleaning. La soglia di “elevata perdita” va passata come filtro esplicito nella query e non è codificata nell'ontologia. |

## Copertura delle RQ

Le CQ coprono RQ1–RQ5 e RQ7 nel knowledge graph; CQ7 supporta RQ6 sull’interlinking. La struttura per età (RQ8 secondaria) è rappresentata nelle osservazioni demografiche (`population0_14`, `population15_64`, `population65Plus`, `share65Plus`) ed è analizzata statisticamente mediante la relazione tra `share65Plus` e quota di record bibliotecari con stato ICCU incluso nel perimetro problematico; i risultati sono riportati in `reports/analysis_tables/age65_problematic_correlation.csv` e nella visualizzazione `visualizations/age65_scatter.png`.